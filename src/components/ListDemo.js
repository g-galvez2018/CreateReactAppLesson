...
export const listItems = numbers.map((unNumero, indice) => <li key={indice}>{unNumero}</li>) ;

export const MoviesList = () => {
  return (
    <ul>
        { 
            movies.map((unaPelicula, indice) =>
            <Card key={indice} title={unaPelicula.title} director={unaPelicula.director} />
            )
